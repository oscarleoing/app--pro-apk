...
class MainActivity8 : AppCompatActivity(), OnMapReadyCallback, OnCategoriaSeleccionadaListener {
    ...
    private var subCategoriaSeleccionadaIcono: Int? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        pestaniaSeleccion = findViewById(R.id.pestaniaSeleccion)
        txtPestania = findViewById(R.id.txtPestania)
        ...
        btnNuevaReporte.apply {
            ...
            setOnClickListener {
                if (layoutGuia.visibility == View.VISIBLE) {
                    ...
                    binding.tvEtiquetaDireccion.visibility = View.VISIBLE
        ...
        try {
            ...
            val request = try {
                AgregarPetconRequest(
                    ...
                    desCalle = selecteddesCalle.orEmptyString(),
                    desFracc = selecteddesFracc.orEmptyString(),
                    numExt = selectedCalleNumero?.trim(),
                    numInt = "",
...
///////
    ...
    private fun mostrarVistaPrevia(reporte: ReporteVistaPrevia) {
        ...
        tvCategoriaVista.text = "Categoría: " + (categoriaSeleccionadaNombre ?: reporte.categoria)
        ...
        tvSubcategoriaVista.text =
            "Subcategoría: " + (subCategoriaSeleccionadaNombre ?: reporte.subcategoria)
        tvDireccionVista.text = "Dirección: " + reporte.direccion
        tvNombreVista.text = "Nombre: " + reporte.nombre
        tvApellidosVista.text = "Apellidos: ${reporte.apellidoPaterno} ${reporte.apellidoMaterno}"
        tvTelefonoVista.text = "Teléfono: " + reporte.telefono
        tvDescripcionVista.text = "Descripción: " + reporte.descripcion
    ...
    override fun onCategoriaSeleccionada(categoria: CategoriaReporteUI) {
        ...
        // MOSTRAR PESTAÑA
        pestaniaSeleccion.visibility = View.VISIBLE

        // CAMBIAR TEXTO
        txtPestania.text = "${categoria.nombre}"

        // CUANDO CAMBIAS DE CATEGORIA → BORRAR SUBCATEGORÍA
    ...
    override fun onSubCategoriaSeleccionada(subCategoria: SubCategoriaReporteUI) {
        ...
        txtPestania.text = "$categoriaSeleccionadaNombre / ${subCategoria.nombre}"

    }


    ///
    fun mostrarBurbujaCategoria(nombre: String, icono: Int, colorSuperior: Int) {

        // Guardar datos
        categoriaSeleccionadaNombre = nombre
        categoriaSeleccionadaIcono = icono
        categoriaSeleccionadaColorSuperior = colorSuperior

        // Mostrar layout contenedor
        layoutBurbujas.visibility = View.VISIBLE
        burbujaCategoria.visibility = View.VISIBLE

        // Aplicar datos
        txtCategoria.text = nombre + subnombre
        imgIconoCategoria.setImageResource(icono)

        val color = ContextCompat.getColor(this, colorSuperior)
        viewColorCategoria2.setBackgroundColor(color)

        // Cuando cambia categoría → ocultar subcategoría
        burbujaSubcategoria.visibility = View.GONE
        subCategoriaSeleccionadaNombre = null
        subCategoriaSeleccionadaIcono = null

        fun mostrarBurbujaSubcategoria(nombre: String, icono: Int) {

            subCategoriaSeleccionadaNombre = nombre
            subnombre = "/ " + nombre
            subCategoriaSeleccionadaIcono = icono
            ...
            imgIconoSubcategoria.setImageResource(icono)
...
////
    ...
    private fun updateDireccionBar(latLng: LatLng) {
        try {
            ...
            if (!addresses.isNullOrEmpty()) {
                val address = addresses[0] // 👈 aquí recibes el objeto Address

                // Mostrar la dirección completa en el TextView
                binding.tvDireccion.text = address.getAddressLine(0)
                // ---- Cerrar teclado al seleccionar un ítem ----
                hideKeyboard(searchText)
                searchText.clearFocus()
                binding.tvDireccion2.text = address.getAddressLine(0)

                // 🔹 Imprimir el objeto completo
                Log.d("DEBUG_ADDRESS", "Objeto Address -> $address")

                // 🔹 Mostrar partes del objeto
                Log.d("DEBUG_ADDRESS", "Max líneas -> ${address.maxAddressLineIndex}")
                for (i in 0..address.maxAddressLineIndex) {
                    Log.d("DEBUG_ADDRESS", "Línea $i -> ${address.getAddressLine(i)}")
                }

                Log.d("DEBUG_ADDRESS", "País -> ${address.countryName}")
                Log.d("DEBUG_ADDRESS", "Estado -> ${address.adminArea}")
                Log.d("DEBUG_ADDRESS", "Ciudad -> ${address.locality}")
                Log.d("DEBUG_ADDRESS", "Colonia -> ${address.subLocality}")
                Log.d("DEBUG_ADDRESS", "Calle -> ${address.thoroughfare}")
                Log.d("DEBUG_ADDRESS", "CP -> ${address.postalCode}")
                Log.d("DEBUG_ADDRESS", "Lat/Lng -> ${address.latitude}, ${address.longitude}")

                // 🔹 También tu TextView
                Log.d("DEBUG_TV", "tvDireccion -> ${binding.tvDireccion}")
                Log.d("DEBUG_TV", "Texto actual -> ${binding.tvDireccion.text}")
            } else {
            ...
            Log.e("DEBUG_ERROR", "Error en updateDireccionBar: ${e.message}")
        }
    }
...
// ============================================================
    ...
    private fun fetchColonias(query: String) {
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                ...
                if (response.isSuccessful) {
                    coloniasList = response.body() ?: emptyList()
                    val nombres = coloniasList.map { it.nomEditFra }
                    ...
                    withContext(Dispatchers.Main) {
                        ...
                        binding.etSearchColonia.setAdapter(adapter)
    ...
    private fun fetchCalles(query: String) {
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                ...
                if (response.isSuccessful) {
                    callesList = response.body() ?: emptyList()
                    val nombres = callesList.map { it.nomEditVia }
                    ...
                    withContext(Dispatchers.Main) {
                        ...
                        binding.etSearchCalle.setAdapter(adapter)
                        adapter.notifyDataSetChanged()
                    }
                }
            } catch (e: Exception) {
    ...
    private fun cambiarAnchoAutoComplete() {
        val autoComplete = findViewById<AutoCompleteTextView>(R.id.etSearchCalle)
        val params = autoComplete.layoutParams
        params.width = 0  // Equivalente a android:layout_width="0dp"
    ...
    private fun restaurarAnchoAutoComplete() {
        val autoComplete = findViewById<AutoCompleteTextView>(R.id.etSearchCalle)
        val params = autoComplete.layoutParams
        params.width = LinearLayout.LayoutParams.MATCH_PARENT // Equivalente a android:layout_width="match_parent"
        autoComplete.layoutParams = params
        binding.etNumeroCalle.visibility = View.GONE
    }
    ...
    private fun setupAutoCompleteColonia() {
        ...
        autoText.addTextChangedListener(object : TextWatcher {
            ...
            override fun onTextChanged(text: CharSequence?, start: Int, before: Int, count: Int) {
                ...
                if (!text.isNullOrEmpty()) fetchColonias(text.toString())
            }
        })

        autoText.setOnItemClickListener { _, _, position, _ ->
            val selected = coloniasList[position]
            autoText.setText(selected.nomEditFra)



            // 🔹 Llamar a buscarUbicacionCalle si existe una calle seleccionada
         //   if (selectedCalleCveViaRepi != null )

         //       buscarUbicacionCalle(selectedCalle!!, selected)
         //   else
            ...
            //
            // 🔹 Llamar a la función según si hay calle seleccionada y número
            if (selectedCalleCveViaRepi != null) {
                if (!binding.etNumeroCalle.text.isNullOrBlank()) {
                    val numeroCalle = binding.etNumeroCalle.text.toString()
                    buscarUbicacionNumero(selectedCalle!!, numeroCalle, selectedColonia)
                } else {
    ...
    private fun setupAutoCompleteCalle() {
        ...
        autoText.addTextChangedListener(object : TextWatcher {
            ...
            override fun onTextChanged(text: CharSequence?, start: Int, before: Int, count: Int) {
                ...
                updateClearButtonVisibility(autoText)
                if (!text.isNullOrEmpty()) fetchCalles(text.toString())
            }
        })

        autoText.setOnItemClickListener { _, _, position, _ ->
            val selected = callesList[position]
            autoText.setText(selected.nomEditVia)
...
// ============================================================
    ...
    private fun agregarFilaBitacora(item: BitacoraResponse) {
        ...
        filaView.findViewById<TextView>(R.id.txtFechaFila).text =
            "Fecha: ${formatearFecha(item.fechaPeticion)}"
        ...
        filaView.findViewById<TextView>(R.id.txtObsFila).text =
            "Observación: ${capitalizarTexto(item.obsPet)}"
        ...
        filaView.findViewById<TextView>(R.id.txtDireccionCompleta).text =
            "Dirección: ${capitalizarTexto(formatearDireccion(item.desCalle, item.desFrac, item.numExt?.trim()))}"
...
// 🔥 Cambiar color sin perder forma redondeada
    ...
    private fun formatearFecha(fecha: String?): String {
        ...
        return try {
            ...
            val salida = SimpleDateFormat("dd MMMM yyyy, h:mm a", Locale("es", "MX"))

            val date = entrada.parse(fecha)
            // formatea (ej: "05 noviembre 2025, 9:29 a. m.") y luego normaliza AM/PM
            val formatted = salida.format(date!!)
            formatted
                .replace("a. m.", "AM")
                .replace("p. m.", "PM")
                .replace("a.m.", "AM")
